...
%Reagan add- some reason ts1 is not only the epoch length, so make it match
%the length of ts1_ex :) -7/21/20 BUG FIX -- need to verify
ts1 = ts1(1:length(ts1_ex));
%//////// 
...
for ii=1:nbEp
  ...
  %//////////RB ADD 7/10/20
     ...
     else %if last one, due to averaging of velocity by bin, have one less value
