function [dev, ratio_strength, dev_min, iweak_pair, istrong_pair] = PP_AssemblyStrength (dev, devControl, data_folder, choice_smooth)
...
elseif choice_smooth = 1
    ...
    devControl_min = zeros(1, number_of_pairs);
    ratio_strength_smooth = zeros(1, number_of_pairs_;
    time = (0:150);
    dev_smoothed = zeros(length(time), number_of_pairs)
     for ipair = 1:number_of_pairs
        dev_smoothed_coeff = polyfit(time, dev(:, ipair)',4);
        dev_smoothed(:, ipair) = polyval(dev_smoothed_coeff, time);
